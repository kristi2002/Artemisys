...
<?php foreach ($futuro as $l):
    ...
    $isOggi = $l['data'] === $oggi;
?>
<div class="stu-card" style="margin-bottom:8px;<?= $isOggi ? 'border:1.5px solid #1e40af;' : '' ?>">
    <a href="<?= BASE_URL ?>studente/lezione/<?= (int)$l['id'] ?>" class="stu-card-link d-flex align-items-center gap-3">
...
?>
<a href="<?= BASE_URL ?>studente/lezione/<?= (int)$l['id'] ?>"
   class="stu-card stu-card-link d-flex align-items-center gap-3" style="margin-bottom:8px;opacity:.88;">
    <div style="flex-shrink:0;width:46px;text-align:center;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;background:#fff;">
